...
! respectively.
! The simulation is performed using the Real Space Renormalization Group procedure. Specifically,
! at each iteration step the system size is doubled and the exact total Hamiltonian of size (2**2N,2**2N) 
! is transformed into an effective Hamiltonian of size (2**N,2**N) by projecting it in the space of the lowest 
! 2**N eigenenergies.
! 
! Example of execution:
! ./RealSpaceRG.out 5 10 input/lambda.dat input/eps.dat
! will find the energy density of the ground state of the system with initial size N=5, until the maximum number of iterations
! = 10 is reached or the updates of the energy goes below the threshold contained in "input/eps.dat". The value of lambda is
! contained in the file "input/lambda.dat". The evolution of the energy density over the iterations is saved into the file
! "results/spectra/spectrum_5_10.dat" and the time needed for the computations is appended to "results/timescalings.csv".
! 
!
! Author: Paolo Zinesi
!
...
PROGRAM RealSpaceRG
      ...
      ELSE
            ...
            WRITE(10, "('NN, Nitermax, lambdafile, epsfile')")
            STOP
      END IF


      ! checks on input validity
      IF ((NN .LT. 2) .OR. (Nitermax .LE. 0) .OR. (eps .LE. 0.D0)) THEN

            ! write error details into an error log file
            PRINT *, "An error occurred, details on 'errors.log'"
            WRITE(10, "('Variables must be `NN`>=2, `Nitermax`>0, and `eps`>0')")
            STOP
      END IF
      ...
      DO WHILE((iterRG .LE. Nitermax) .AND. (ABS(energy_update) .GT. eps))
            ...
            CALL DSYEVX('V', 'I', &                         ! compute eigenvalues and eigenvectors with index in [IL,IU]
                        'L', 2**(2*NN), mat2N, 2**(2*NN), & ! store lower triangular matrix of 2**(2*NN) x 2**(2*NN) matrix H2N
            ...
            energy_update = energydensity - HN(1,1)/NN
            energydensity = HN(1,1)/NN


            ! write ground state energy for the given iterRG
            WRITE(20, "(ES24.17, ' ')", advance="no") energydensity
